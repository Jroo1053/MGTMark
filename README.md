...
A high(ish)-performance MGT (Machine Generated Text) obfuscation and detection benchmarking tool, supporting HuggingFace pipelines. This software allows users to test commercial and open-source MGT classifiers against AI generated texts, with and without authorship obfuscation. Datasets and models can be pulled straight from HuggingFace and then obfuscated with a variety of configurable attacks.

This should provide a strong indication of which detection systems are affective and whether they can be defeated through authorship obfuscation. This is actually quite important. LLMs are here to stay, and detection will be a major determining factor on their impact on society. Continual and thorough assessment of MGT classifiers is crucial and authorship obfuscation must be considered alongside this. After all if a basic attack can render a classifier completely useless, then should it be considered affective in a more general sense?
...
7. **Article Deletion** - Deletes certain words from the text. Best used to delete articles i.e The or And.
8. **Paragraph Addition** - Doubles newlines
9. **Whitespace Addition** - Randomly double whitespace
10. **Alter Numbers** - Alter numbers in text ie. 1984 -> 3485

**Credit** Attacks 7-10 follow the implementation in RAID[https://github.com/liamdugan/raid]
...
V2 adds the ability to define separate runs and chain attacks together. To do so, simply add attacks in a list using the following format. **note** attacks will be applied as is, which will result in compatibility issues. Generally advisable to run the most aggressive attacks last.

```json
  "attack_configs": [
      ["upper_lower", "article","paragraph","whitespace","spelling","glyph","zwsp"],
      ["upper_lower"]
  ]
```
